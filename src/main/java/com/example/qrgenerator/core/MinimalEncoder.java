 ...
 * @see https://github.com/zxing/zxing/blob/master/core/src/main/java/com/google/zxing/qrcode/encoder/MinimalEncoder.java
 */
final class MinimalEncoder {
  ...
  ResultList encode(Version version) throws WriterException {
    if (version == null) { // compute minimal encoding trying the three version sizes.
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (ScQREncoder.willFit(size, versions[i], ecLevel) && size < smallestSize) {
          smallestSize = size;
          smallestResult = i;
        }
      }
      ...
      if (!ScQREncoder.willFit(result.getSize(), getVersion(getVersionSize(result.getVersion())), ecLevel)) {
  ...
  static boolean isDoubleByteKanji(char c) {
    return ScQREncoder.isOnlyDoubleByteKanji(String.valueOf(c));
  }

  static boolean isAlphanumeric(char c) {
    return ScQREncoder.getAlphanumericCode(c) != -1;
  }
  ...
  final class ResultList {
    ...
    ResultList(Version version, Edge solution) {
      ...
      while (versionNumber < upperLimit && !ScQREncoder.willFit(size, Version.getVersionForNumber(versionNumber),
        ecLevel)) {
        versionNumber++;
      }
      // shrink version if possible
      while (versionNumber > lowerLimit && ScQREncoder.willFit(size, Version.getVersionForNumber(versionNumber - 1),
        ecLevel)) {
        versionNumber--;
      }
    ...
    final class ResultNode {
      ...
      private void getBits(BitArray bits) throws WriterException {
        ...
        if (mode == Mode.ECI) {
          ...
          // append data
          ScQREncoder.appendBytes(stringToEncode.substring(fromPosition, fromPosition + characterLength), mode, bits,
